...
public class SimpleTransform {
	...
	public Dataset<Row> latlongYearly(boolean condition)
	{
		Dataset<Row> first_tran=spark.sql("select DISTINCT(NewGeoreferencedColumn), County, ReportingYear, SUM(GasProduced_Mcf), "
				...
				+"where (GasProduced_Mcf is not null OR WaterProduced_bbl is not null OR "
				+"OilProduced_bbl is not null OR NewGeoreferencedColumn is not null) "
	...
	{
		Dataset<Row> result= spark.sql("SELECT DISTINCT(CompanyName),ReportingYear AS Year, SUM(GasProducedMcf)Total_GAS, "
